...
def train_model(model, train_loader, device, face_detector, smpl_model, epochs=1, log_interval=10):
    ...
    for epoch in range(epochs):
        ...
        save_path = f"./final_model_ft_epoch_{epoch + 1}.pth"
...
if __name__ == "__main__":
    ...
    model = Model()
    try:
        state_dict = torch.load('model_epoch_100.pth', map_location=device)
        if isinstance(state_dict, dict):
            model.flownet.load_state_dict(state_dict)
        elif isinstance(state_dict, Model):
            model.flownet.load_state_dict(state_dict.flownet.state_dict())
